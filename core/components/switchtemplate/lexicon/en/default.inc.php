<?php
/**
 * Default Lexicon Entries for SwitchTemplate
 *
 * @package switchtemplate
 * @subpackage lexicon
 */
$_lang['switchtemplate'] = 'SwitchTemplate';
$_lang['switchtemplate_desc'] = 'Switch resource templates on the fly';

$_lang['switchtemplate.menu_home'] = 'SwitchTemplate';
$_lang['switchtemplate.menu_home_desc'] = 'Switch resource templates on the fly';

$_lang['switchtemplate.cache'] = 'Cache the Output';
$_lang['switchtemplate.cache_short'] = 'Cache';
$_lang['switchtemplate.debug_mode'] = 'Debug Mode';
$_lang['switchtemplate.exclude'] = 'Disabled Resources';
$_lang['switchtemplate.exclude_desc'] = 'Empty: \'None Disabled\'';
$_lang['switchtemplate.extension'] = 'Extension';
$_lang['switchtemplate.include'] = 'Enabled Resources';
$_lang['switchtemplate.include_desc'] = 'Empty: \'All Enabled\'';
$_lang['switchtemplate.key'] = 'Key';
$_lang['switchtemplate.settings'] = 'Settings';
$_lang['switchtemplate.settings_desc'] = 'Create and modify your SwitchTemplate settings, that are used to change the the template of a MODX resource on the fly.';
$_lang['switchtemplate.name'] = 'Name';
$_lang['switchtemplate.output'] = 'Output Type';
$_lang['switchtemplate.output_amp'] = 'AMP';
$_lang['switchtemplate.output_empty'] = 'Select Output Type';
$_lang['switchtemplate.output_html'] = 'HTML';
$_lang['switchtemplate.output_short'] = 'Output';
$_lang['switchtemplate.output_template'] = 'Template';
$_lang['switchtemplate.search'] = 'Search';
$_lang['switchtemplate.setting_ceate'] = 'Create';
$_lang['switchtemplate.setting_create'] = 'New Setting';
$_lang['switchtemplate.setting_remove'] = 'Delete';
$_lang['switchtemplate.setting_remove_confirm'] = 'Are you sure you want to delete this SwitchTemplate setting?';
$_lang['switchtemplate.setting_update'] = 'Update';
$_lang['switchtemplate.templatename'] = 'Chunk/Template Name';
$_lang['switchtemplate.type'] = 'Template Type';
$_lang['switchtemplate.type_chunk'] = 'Chunk';
$_lang['switchtemplate.type_empty'] = 'Select Template Type';
$_lang['switchtemplate.type_short'] = 'Type';
$_lang['switchtemplate.type_template'] = 'Template';

$_lang['switchtemplate.err_chunk_nf'] = 'Chunk \'[[+name]]\' is not found.';
$_lang['switchtemplate.err_template_invalid'] = 'Template is not valid.';
$_lang['switchtemplate.err_template_nf'] = 'Template \'[[+name]]\' is not found.';

$_lang['switchtemplate.setting_err_invalid'] = 'Invalid data.';
$_lang['switchtemplate.setting_err_nf'] = 'Setting not found.';
$_lang['switchtemplate.setting_err_ns'] = 'Setting not specified.';
$_lang['switchtemplate.setting_err_save'] = 'An error occurred while trying to save the setting.';
